...
//
// specmon plots for Modulation data
//
// Usage: From root command line type ".x specmon.C(runname, plot_type, channel, log_scale, save_plot)"
//
...
/*----------------------------------------------------------------------------*/
void specmon(string runname, string plot_type, int ichannel, bool log_scale, bool save_plot)
{
    ...
    if        (plot_type == "spectrum") { // 1D energy spectrum
        ...
        cout<< "Wrong plot type selected..... look at documentation in specmon.C"<<endl;
        return;
    }
